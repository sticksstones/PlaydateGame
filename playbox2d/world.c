...
PBWorld* PBWorldCreate(PBVec2 gravity, int iterations) {
  ...
  world->angularVelocityDampening = 1.0;
...
void PBWorldStep(PBWorld* world, float dt) {
  ...
  for(int i = 0; i < world->bodies->count; ++i) {
    ...
    b->velocity = PBVec2Add(b->velocity, PBVec2MultF(PBVec2Add(PBVec2MultF(world->gravity, b->gravityMult), PBVec2MultF(b->force, b->invMass)), dt));
    b->angularVelocity += dt * b->invI * b->torque;
    b->angularVelocity *= pow(world->angularVelocityDampening, dt);
  }
