...
int playbox_body_setLockPosition(lua_State* L) {
PBBody* body = getBodyArg(1);
body->lockPosition = pd->lua->getArgInt(2);
return 0;
}

int playbox_body_setGravityMult(lua_State* L) {
PBBody* body = getBodyArg(1);
body->gravityMult = pd->lua->getArgFloat(2);
return 0;
}
...
int playbox_body_getTorque(lua_State* L) {
  PBBody* body = getBodyArg(1);
  pd->lua->pushFloat(body->torque);
  return 1;  
}
...
{ "setLockPosition", playbox_body_setLockPosition },
{ "setGravityMult", playbox_body_setGravityMult },
...
{ "getTorque", playbox_body_getTorque },
...
int playbox_world_setAngularVelocityDampening(lua_State* L) {
  PBWorld* world = getWorldArg(1);
  float angularVelocityDampening = pd->lua->getArgFloat(2);
  world->angularVelocityDampening = angularVelocityDampening;
  return 0;
}
...
{ "setAngularVelocityDampening", playbox_world_setAngularVelocityDampening },
